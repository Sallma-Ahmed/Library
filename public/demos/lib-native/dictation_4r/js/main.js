new Vue({
  ...
  methods: {
        ...
        ? fetch(pageUrl + ".json")
          ...
          .then((data) => {
            ...
            this.posts[0].partArray.map(function (element, i) {
              ...
              comparison_player.resultWord = false;
    ...
    clickstart() {
      ...
      finalResponse.submitData(this.posts, 1, this.posts[0].counterCorrect);
    ...
    answer_click: function () {
      ...
      if (this.posts[0].partArray.length - 1 == this.posts[0].countItem) {
        ...
        if (percent >= 85) {
          ...
          finalResponse.submitData(this.posts, 4, this.posts[0].counterCorrect);
        }
      } else {
    ...
    updateActiveOptionOnTouchMove(event) {
      ...
      let optionsElements = Array.from(this.$el.querySelectorAll('.option-popup button'));
    ...
    getLetterImage(option) {
      return `../../../lib-native/dictation_4r/images/${option.id}.png`;
    }
